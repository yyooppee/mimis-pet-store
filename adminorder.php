...
if (isset($_SESSION['ad_id'])) {
    echo "<script>alert('emp_id is set: " . $_SESSION['ad_id'] . "')</script>";
    $_SESSION['ad_id_alert_shown'] = true; // Set the flag to true
} else {
    // Uncomment this line if you want to show the alert when not set
    // echo "<script>alert('emp_id is NOT set')</script>"; 
}
...
<body>
    ...
    foreach ($result as $row) {
        ...
        if (isset($_POST[$button_name])) {
            ...
            $emp_id = $_SESSION['ad_id']; // Assign the admin ID to the emp_id variable
            ...
            if ($result_prod_id) {
                ...
                if ($row_prod_id = mysqli_fetch_assoc($result_prod_id)) {
                    ...
                    $sql_check_invoice = "SELECT invo_id FROM invoice WHERE emp_id = $emp_id AND invo_stat = 'open'";
